...
| 12 | GST Authentication | verify-gst | DONE ✅ | 2026-08-19 · 2 scenarios (additionalData true/false) as variants[]. 2026-08-20 · Fixed a real vendor-routing bug found via live testing: the GST family does NOT use the testapi.karza.in/api.karza.in host-swap every other endpoint uses — it always calls `api.karza.in` (same host as prod) with the environment baked into the path segment instead (`uat` for test, `prod` for live). Confirmed via the vendor's own Postman collection ("GST Authentication collection.json") + a live call returning real data. Added dedicated `KARZA_GST_BASE_URL` in `karza.ts`; `route.ts` now fetches `${KARZA_GST_BASE_URL}${KARZA_GST_PATH}` instead of `${KARZA_BASE_URL}${KARZA_GST_PATH}`. Also fixed `additionalData` rendering as free text instead of the Yes/No toggle (missing `enum: ['true','false']`). |
| 13 | GST Advanced | verify-gst-advanced | DONE ✅ | 2026-08-19 · 2 scenarios (liabilityDetails true/false); result[] array of GSTIN entries with nested profile+filingStatus, shared field-tree helper (gstinEntryFields) to avoid duplicating ~50 nested paths. 2026-08-20 · Same `KARZA_GST_BASE_URL` fix as row 12 applied (shares the `/gst/{env}/v2/...` path family) — verified live with a real 200 response, not just inferred. |
| 14 | GST Search Basis PAN | verify-gst-by-pan | DONE ✅ | 2026-08-19 · single scenario; lightweight GSTIN-lookup counterpart to GST Advanced. 2026-08-20 · Same `KARZA_GST_BASE_URL` bug as rows 12/13 (was hardcoded `/gst/prod/v2/search` on `KARZA_BASE_URL`/testapi.karza.in → 403 "Missing Authentication Token"). Fixed to `${KARZA_GST_BASE_URL}` (api.karza.in) + env-derived path segment; verified live with a real 200 response and actual GSTIN data. |
