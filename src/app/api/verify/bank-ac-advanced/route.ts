...
import {
  KARZA_BASE_URL,
  KARZA_AUTH_HEADER,
  KARZA_BANK_AC_ADVANCED_PATH,
  redactPii,
} from '@/lib/vendors/karza'
import { logApiCall } from '@/lib/vendors/log'

const ENDPOINT = '/api/verify/bank-ac-advanced'
...
const BodySchema = z.object({
  ...
  accountNumber:           z.string().min(5).max(25).regex(/^[a-zA-Z0-9]+$/, 'Invalid account number'),
  accountHolderName:       z.string().regex(/^[a-zA-Z0-9&,\-/()_'. ]+$/, 'Invalid account holder name').optional(),
  multiNameList:           z.array(z.string()).optional(),
  ifsc:                    z.string().regex(/^[\w]{4}0[\w|\d]{6}$/, 'Invalid IFSC code'),
  nameMatchType:           z.enum(['individual', 'entity', 'Individual', 'Entity']).optional(),
  useCombinedSolution:     z.enum(['Y']).optional(),
  allowPartialMatch:       z.boolean().optional(),
  preset:                  z.enum(['G', 'L', 'S']).optional(),
  suppressReorderPenalty:  z.boolean().optional(),
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!karzaKey) {
      statusCode = 503
      return NextResponse.json(
        { success: false, error: 'Bank account verification is not configured.', code: 'VENDOR_NOT_CONFIGURED' },
        { status: 503 }
      )
    }
    ...
    try {
      vendorRes = await fetch(`${KARZA_BASE_URL}${KARZA_BANK_AC_ADVANCED_PATH}`, {
        method: 'POST',
        headers: {
      ...
      return NextResponse.json(
        {
          success: false,
          error: aborted ? 'Bank account verification timed out.' : 'Bank account verification is temporarily unavailable.',
    ...
    if (!vendorRes.ok) {
      ...
      return NextResponse.json(
        {
          success: false,
          error: 'Bank account verification failed at the provider.',
          code: 'VENDOR_ERROR',
