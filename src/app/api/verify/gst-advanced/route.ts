...
const ENDPOINT = '/api/verify/gst-advanced'
...
const BodySchema = z.object({
  ...
  liabilityDetails: z.boolean().optional(),
  stateCode:        z.array(z.string().regex(/^\d{2}$/, 'stateCode entries must be 2 digits')).optional(),
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!karzaKey) {
      statusCode = 503
      return NextResponse.json(
        { success: false, error: 'GST advanced lookup is not configured.', code: 'VENDOR_NOT_CONFIGURED' },
        { status: 503 }
      )
    }
    ...
    try {
      ...
      return NextResponse.json(
        {
          success: false,
          error: aborted ? 'GST advanced lookup timed out.' : 'GST advanced lookup is temporarily unavailable.',
    ...
    if (!vendorRes.ok) {
      ...
      return NextResponse.json(
        {
          success: false,
          error: 'GST advanced lookup failed at the provider.',
          code: 'VENDOR_ERROR',
