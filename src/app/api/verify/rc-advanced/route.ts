...
const BodySchema = z.object({
  ...
  registrationNumber: z.string().min(6).max(20),
  version:            z.number().gt(3),
  partialEngine:      z.enum(['Y', 'N']).optional(),
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!karzaKey) {
      statusCode = 503
      return NextResponse.json(
        { success: false, error: 'Vehicle RC authentication is not configured.', code: 'VENDOR_NOT_CONFIGURED' },
        { status: 503 }
      )
    }
    ...
    try {
      vendorRes = await fetch(`${KARZA_BASE_URL}${KARZA_RC_ADVANCED_PATH}`, {
        method: 'POST',
        headers: {
      ...
      return NextResponse.json(
        {
          success: false,
          error: aborted ? 'Vehicle RC authentication timed out.' : 'Vehicle RC authentication is temporarily unavailable.',
    ...
    if (!vendorRes.ok) {
      ...
      return NextResponse.json(
        {
          success: false,
          error: 'Vehicle RC authentication failed at the provider.',
          code: 'VENDOR_ERROR',
