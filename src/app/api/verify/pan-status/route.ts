...
const BodySchema = z.object({
  ...
  name:    z.string().min(1),
  dob:     z.string().regex(/^\d{1,2}\/\d{1,2}\/\d{4}$/, 'dob must be DD/MM/YYYY'),
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!karzaKey) {
      statusCode = 503
      return NextResponse.json(
        { success: false, error: 'PAN status check is not configured.', code: 'VENDOR_NOT_CONFIGURED' },
        { status: 503 }
      )
    }
    ...
    try {
      vendorRes = await fetch(`${KARZA_BASE_URL}${KARZA_PAN_STATUS_PATH}`, {
        method: 'POST',
        headers: {
      ...
      return NextResponse.json(
        {
          success: false,
          error: aborted ? 'PAN status check timed out.' : 'PAN status check is temporarily unavailable.',
    ...
    if (!vendorRes.ok) {
      ...
      return NextResponse.json(
        {
          success: false,
          error: 'PAN status check failed at the provider.',
          code: 'VENDOR_ERROR',
