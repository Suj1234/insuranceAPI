...
import {
  KARZA_BASE_URL,
  KARZA_AUTH_HEADER,
  KARZA_PAN_LINK_UNIQUE_CONSENT_PATH,
  redactPii,
} from '@/lib/vendors/karza'
import { logApiCall } from '@/lib/vendors/log'

const ENDPOINT = '/api/verify/pan-link-unique-consent'
const VENDOR_TIMEOUT_MS = 30_000

// Public request body — mirrors the PDF's Request (Share Consent) schema exactly.
// Every field the tryout can send MUST be listed here (Zod strips unknowns → dropped).
const BodySchema = z.object({
  ...
  name:        z.string().min(1),
  consentTime: z.string(),
  consentText: z.string().max(10000),
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!karzaKey) {
      statusCode = 503
      return NextResponse.json(
        { success: false, error: 'PAN link consent is not configured.', code: 'VENDOR_NOT_CONFIGURED' },
        { status: 503 }
      )
    }
    ...
    try {
      vendorRes = await fetch(`${KARZA_BASE_URL}${KARZA_PAN_LINK_UNIQUE_CONSENT_PATH}`, {
        method: 'POST',
        headers: {
      ...
      return NextResponse.json(
        {
          success: false,
          error: aborted ? 'PAN link consent timed out.' : 'PAN link consent is temporarily unavailable.',
    ...
    if (!vendorRes.ok) {
      ...
      return NextResponse.json(
        {
          success: false,
          error: 'PAN link consent failed at the provider.',
          code: 'VENDOR_ERROR',
