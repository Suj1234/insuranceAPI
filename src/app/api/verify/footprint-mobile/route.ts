...
import {
  KARZA_BASE_URL,
  KARZA_AUTH_HEADER,
  KARZA_DIGITAL_FOOTPRINT_MOBILE_PATH,
  redactPii,
} from '@/lib/vendors/karza'
import { logApiCall } from '@/lib/vendors/log'

const ENDPOINT = '/api/verify/footprint-mobile'
...
const BodySchema = z.object({
  ...
  mobile:                     z.string().regex(/^[6-9][0-9]{9}$/, 'Invalid mobile number'),
  phoneDetails:               z.enum(['Y', 'N']).optional(),
  statutoryPresenceRequired:  z.enum(['Y', 'N']).optional(),
  financialPresenceRequired:  z.enum(['Y', 'N']).optional(),
  rcLinkageRequired:          z.enum(['Y', 'N']).optional(),
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!karzaKey) {
      statusCode = 503
      return NextResponse.json(
        { success: false, error: 'Digital footprint (mobile) check is not configured.', code: 'VENDOR_NOT_CONFIGURED' },
        { status: 503 }
      )
    }
    ...
    try {
      vendorRes = await fetch(`${KARZA_BASE_URL}${KARZA_DIGITAL_FOOTPRINT_MOBILE_PATH}`, {
        method: 'POST',
        headers: {
      ...
      return NextResponse.json(
        {
          success: false,
          error: aborted ? 'Digital footprint check timed out.' : 'Digital footprint check is temporarily unavailable.',
    ...
    if (!vendorRes.ok) {
      ...
      return NextResponse.json(
        {
          success: false,
          error: 'Digital footprint check failed at the provider.',
          code: 'VENDOR_ERROR',
