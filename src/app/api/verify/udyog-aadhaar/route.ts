...
import {
  KARZA_BASE_URL,
  KARZA_AUTH_HEADER,
  KARZA_UDYOG_AADHAAR_PATH,
  redactPii,
} from '@/lib/vendors/karza'
import { logApiCall } from '@/lib/vendors/log'

const ENDPOINT = '/api/verify/udyog-aadhaar'
...
const BodySchema = z.object({
  ...
  uan:        z.string().length(12).regex(/^[A-Z]{2}\d{2}[A-Z]{1}\d{7}$/, 'Invalid UAN format'),
  mobile:     z.string().regex(/^[6-9][0-9]{9}$/, 'Invalid mobile number').optional().or(z.literal('')),
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!karzaKey) {
      statusCode = 503
      return NextResponse.json(
        { success: false, error: 'Udyog Aadhaar authentication is not configured.', code: 'VENDOR_NOT_CONFIGURED' },
        { status: 503 }
      )
    }
    ...
    try {
      vendorRes = await fetch(`${KARZA_BASE_URL}${KARZA_UDYOG_AADHAAR_PATH}`, {
        method: 'POST',
        headers: {
      ...
      return NextResponse.json(
        {
          success: false,
          error: aborted ? 'Udyog Aadhaar authentication timed out.' : 'Udyog Aadhaar authentication is temporarily unavailable.',
    ...
    if (!vendorRes.ok) {
      ...
      return NextResponse.json(
        {
          success: false,
          error: 'Udyog Aadhaar authentication failed at the provider.',
          code: 'VENDOR_ERROR',
