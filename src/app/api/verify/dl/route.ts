...
const BodySchema = z.object({
  ...
  dlNo:              z.string().min(9).max(50),
  dob:               z.string().regex(/^(0[1-9]|[12][0-9]|3[01])-(0[1-9]|1[012])-(19|20)\d\d$/, 'dob must be DD-MM-YYYY'),
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!karzaKey) {
      statusCode = 503
      return NextResponse.json(
        { success: false, error: "Driver's License authentication is not configured.", code: 'VENDOR_NOT_CONFIGURED' },
        { status: 503 }
      )
    }
    ...
    try {
      vendorRes = await fetch(`${KARZA_BASE_URL}${KARZA_DL_PATH}`, {
        method: 'POST',
        headers: {
      ...
      return NextResponse.json(
        {
          success: false,
          error: aborted ? "Driver's License authentication timed out." : "Driver's License authentication is temporarily unavailable.",
    ...
    if (!vendorRes.ok) {
      ...
      return NextResponse.json(
        {
          success: false,
          error: "Driver's License authentication failed at the provider.",
          code: 'VENDOR_ERROR',
