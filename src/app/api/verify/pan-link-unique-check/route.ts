...
import {
  KARZA_BASE_URL,
  KARZA_AUTH_HEADER,
  KARZA_PAN_LINK_UNIQUE_CHECK_PATH,
  redactPii,
} from '@/lib/vendors/karza'
import { logApiCall } from '@/lib/vendors/log'

const ENDPOINT = '/api/verify/pan-link-unique-check'
const VENDOR_TIMEOUT_MS = 30_000

// Public request body — mirrors the PDF's Request (PAN Aadhaar Link Status) schema
// exactly. Every field the tryout can send MUST be listed here (Zod strips unknowns).
const BodySchema = z.object({
  consent:    z.enum(['Y', 'N']).default('Y'),
  pan:        z.string().regex(/^[A-Za-z]{5}\d{4}[A-Za-z]{1}$/, 'Invalid PAN format'),
  aadhaarNo:  z.string().regex(/^[0-9]{12}$/, 'aadhaarNo must be 12 digits'),
  accessKey:  z.string().min(1),
  clientData: z.object({ caseId: z.string().max(200) }),
})
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!karzaKey) {
      statusCode = 503
      return NextResponse.json(
        { success: false, error: 'PAN link status check is not configured.', code: 'VENDOR_NOT_CONFIGURED' },
        { status: 503 }
      )
    }
    ...
    try {
      vendorRes = await fetch(`${KARZA_BASE_URL}${KARZA_PAN_LINK_UNIQUE_CHECK_PATH}`, {
        method: 'POST',
        headers: {
      ...
      return NextResponse.json(
        {
          success: false,
          error: aborted ? 'PAN link status check timed out.' : 'PAN link status check is temporarily unavailable.',
    ...
    if (!vendorRes.ok) {
      ...
      return NextResponse.json(
        {
          success: false,
          error: 'PAN link status check failed at the provider.',
          code: 'VENDOR_ERROR',
