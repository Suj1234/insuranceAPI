...
import {
  KARZA_BASE_URL,
  KARZA_AUTH_HEADER,
  KARZA_DIGITAL_FOOTPRINT_EMAIL_PATH,
  redactPii,
} from '@/lib/vendors/karza'
import { logApiCall } from '@/lib/vendors/log'

const ENDPOINT = '/api/verify/footprint-email'
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!karzaKey) {
      statusCode = 503
      return NextResponse.json(
        { success: false, error: 'Digital footprint (email) check is not configured.', code: 'VENDOR_NOT_CONFIGURED' },
        { status: 503 }
      )
    }
    ...
    try {
      vendorRes = await fetch(`${KARZA_BASE_URL}${KARZA_DIGITAL_FOOTPRINT_EMAIL_PATH}`, {
        method: 'POST',
        headers: {
