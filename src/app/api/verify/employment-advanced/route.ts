...
import {
  KARZA_BASE_URL,
  KARZA_AUTH_HEADER,
  KARZA_EMPLOYMENT_ADVANCED_PATH,
  redactPii,
} from '@/lib/vendors/karza'
import { logApiCall } from '@/lib/vendors/log'

const ENDPOINT = '/api/verify/employment-advanced'
const VENDOR_TIMEOUT_MS = 290_000
...
const BodySchema = z.object({
  ...
  uans:             z.array(z.string().regex(/^[0-9]{12}$/, 'Invalid UAN')).optional(),
  entityId:         z.string().optional(),
  employerName:     z.string().min(3).max(255).optional(),
  employeeName:     z.string().min(3).max(50).optional(),
  mobile:           z.string().regex(/^[6-9]\d{9}$/, 'Invalid mobile number').optional(),
  emailId:          z.string().email().optional(),
  runPanFlow:       z.boolean(),
  pan:              z.string().length(10).regex(/^[A-Za-z]{5}\d{4}[A-Za-z]{1}$/, 'Invalid PAN format'),
  showFailures:     z.boolean().optional(),
  isLatestEmployer: z.boolean().optional(),
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!karzaKey) {
      statusCode = 503
      return NextResponse.json(
        { success: false, error: 'Employment verification is not configured.', code: 'VENDOR_NOT_CONFIGURED' },
        { status: 503 }
      )
    }
    ...
    try {
      vendorRes = await fetch(`${KARZA_BASE_URL}${KARZA_EMPLOYMENT_ADVANCED_PATH}`, {
        method: 'POST',
        headers: {
      ...
      return NextResponse.json(
        {
          success: false,
          error: aborted ? 'Employment verification timed out.' : 'Employment verification is temporarily unavailable.',
    ...
    if (!vendorRes.ok) {
      ...
      return NextResponse.json(
        {
          success: false,
          error: 'Employment verification failed at the provider.',
          code: 'VENDOR_ERROR',
