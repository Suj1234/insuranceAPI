...
const ENDPOINT = '/api/verify/passport'
...
const BodySchema = z.object({
  ...
  passportStatus: z.enum(['Y', 'N']).optional(),
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!karzaKey) {
      statusCode = 503
      return NextResponse.json(
        { success: false, error: 'Passport verification is not configured.', code: 'VENDOR_NOT_CONFIGURED' },
        { status: 503 }
      )
    }
    ...
    try {
      vendorRes = await fetch(`${KARZA_BASE_URL}${KARZA_PASSPORT_PATH}`, {
        method: 'POST',
        headers: {
      ...
      return NextResponse.json(
        {
          success: false,
          error: aborted ? 'Passport verification timed out.' : 'Passport verification is temporarily unavailable.',
    ...
    if (!vendorRes.ok) {
      ...
      return NextResponse.json(
        {
          success: false,
          error: 'Passport verification failed at the provider.',
          code: 'VENDOR_ERROR',
