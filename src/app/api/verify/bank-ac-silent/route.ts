...
import {
  KARZA_BASE_URL,
  KARZA_AUTH_HEADER,
  KARZA_BANK_AC_SILENT_PATH,
  redactPii,
} from '@/lib/vendors/karza'
import { logApiCall } from '@/lib/vendors/log'

const ENDPOINT = '/api/verify/bank-ac-silent'
...
const BodySchema = z.object({
  ...
  nameMatchType:          z.string().optional(),
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!karzaKey) {
      statusCode = 503
      return NextResponse.json(
        { success: false, error: 'Silent bank account verification is not configured.', code: 'VENDOR_NOT_CONFIGURED' },
        { status: 503 }
      )
    }
    ...
    try {
      vendorRes = await fetch(`${KARZA_BASE_URL}${KARZA_BANK_AC_SILENT_PATH}`, {
        method: 'POST',
        headers: {
      ...
      return NextResponse.json(
        {
          success: false,
          error: aborted ? 'Silent bank account verification timed out.' : 'Silent bank account verification is temporarily unavailable.',
    ...
    if (!vendorRes.ok) {
      ...
      return NextResponse.json(
        {
          success: false,
          error: 'Silent bank account verification failed at the provider.',
          code: 'VENDOR_ERROR',
