...
import {
  KARZA_BASE_URL,
  KARZA_AUTH_HEADER,
  KARZA_PAN_LINK_ANY_PATH,
  redactPii,
} from '@/lib/vendors/karza'
import { logApiCall } from '@/lib/vendors/log'

const ENDPOINT = '/api/verify/pan-link-any'
...
const BodySchema = z.object({
  pan:        z.string().regex(/^[A-Za-z]{3}[Pp][A-Za-z][0-9]{4}[A-Za-z]$/, 'Invalid PAN format'),
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    try {
      vendorRes = await fetch(`${KARZA_BASE_URL}${KARZA_PAN_LINK_ANY_PATH}`, {
        method: 'POST',
        headers: {
